...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const storedCount = localStorage.getItem('infiniteWikiSearchCount');
    if (storedCount) {
    ...
    if (typeof process !== 'undefined' && !process.env.API_KEY) {
      setError("No Gemini API key found in env file.");
      setIsLoading(false);
      return;
    }
    ...
    const fetchContentAndArt = async () => {
      ...
      localStorage.setItem('infiniteWikiSearchCount', newCount.toString());
  ...
  const handleSearch = useCallback((topic: string) => {
    ...
    if (newTopic && newTopic.toLowerCase() !== currentTopic.toLowerCase()) {
      ...
      localStorage.setItem('infiniteWikiSearchCount', (searchCount + 1).toString());
    }
  }, [currentTopic, searchCount]);
    ...
    <div>
      ...
      <footer className="sticky-footer">
        <p className="footer-text" style={{ margin: 0 }}>
          ❀ <a href="https://github.com/namandhakad712/Infinte-Word-Search" target="_blank" rel="noopener noreferrer">Infinite Wiki</a> · Generated by Gemini 2.5 Flash Lite
          {generationTime && ` · ${Math.round(generationTime)}ms`}
          <span className="search-count"> · Searches: {searchCount}</span>
        </p>
      </footer>
    </div>
  );
